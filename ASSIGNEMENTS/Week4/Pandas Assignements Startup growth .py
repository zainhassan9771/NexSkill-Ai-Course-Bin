...
df=pd.read_csv("Week6/startup_growth_investment_data.csv",delimiter=",")
print(df)
...
# access the Startup Name column
status=df["Startup Name"]
print("Access the Startup Name Coloumn: df: ",status)
print()

#Access multiple coloumn
bed_bath=df[["Industry","Funding Rounds"]]
...
second_row7=df.loc[:1,"Startup Name":"Industry"]
print("Slicing colouns are :",second_row7)

#Combined row and column selection using .loc
second_row8=df.loc[df["Industry"]=="SaaS","Country"]
...
df_index_col=pd.read_csv("Week6/startup_growth_investment_data.csv",delimiter=",",index_col=("Startup Name"))
...
#Selecting a single row using .loc
SingleRow=df_index_col.loc["Startup_1"]
print("Selecting a single row :",SingleRow)

#Selecting multiple rows using .loc
MultipleRows2=df_index_col.loc[["Startup_2","Startup_3"]]
print(MultipleRows2)

#Selecting a slice of rows using .loc
SliceRow=df_index_col.loc["Startup_1":"Startup_10"]
print(SliceRow)

#Conditional selection of rows using .loc
SelectionRow=df_index_col.loc[df_index_col["Industry"]=="SaaS"]
print("Conditional selection of rows using .loc")
print(SelectionRow)

#Selecting a single column using .loc
SingleColoumn=df_index_col.loc["Startup_1":"Startup_10","Industry"]
print("Selecting a single column using .loc")
print(SingleColoumn)

#Selecting multiple columns using .loc
MultipleColoumns=df_index_col.loc["Startup_1":"Startup_10",["Valuation (USD)","Country"]]
print("Selecting multiple columns using .loc")
print(MultipleColoumns)

#Selecting a slice of columns using .loc
SliceColoumn=df_index_col.loc["Startup_1":"Startup_10","Industry":"Country"]
print("Selecting a slice of columns using .loc")
print(SliceColoumn)

#Combined row and column selection using .loc
Combined=df_index_col.loc[df_index_col["Country"]=="Germany","Investment Amount (USD)":"Country"]
...
#delete Funding Rounds and Industry coloumn
df.drop(["Funding Rounds","Industry"], axis=1, inplace=True)
print(df)

#Rename Labels in a DataFrame
#Rename column Investment Amount (USD) to new_investment
df.rename(columns={"Investment Amount (USD)":"new_investment"},inplace=True)
print(df)
